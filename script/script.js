...
async function fetchAllSongs() {
    let response = await fetch('/songs/'); // Fetch directory structure
    ...
    for (let folder of folders) {
        ...
        if (folder.href.endsWith("/")) {
            let songsResponse = await fetch(`/songs/${folderName}/`);
...
async function fetchFolders() {
    ...
    Array.from(folders).forEach(folder => {
        ...
        if (folder.href.endsWith("/") && !folderName.startsWith("127.0.0.1:3000")) {
            ...
            folderDiv.innerHTML= `<img src="songs/${folderName}/image.jpeg" alt="${folderName}"><div class="ArtistName">${folderName.replaceAll("%20"," ")}</div>` ;
